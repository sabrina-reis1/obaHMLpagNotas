...
$(document).ready(function () {
    ...
    if($('#sw_ccusto_aprovador')[0].checked != true){
        $('#div_inputCCA').hide()
    }else{
        $('#div_inputCCA').show()
    }
    ...
    if (state == '0' || state == '5') {
        ...
        //AJUSTADO - setar no campo #aprovAtual o primeiro aprovador da tabela pai x filho de aprovadores 
        const aprovadores = Array.from($('[id*=txt_IdAprovador___]'));
        $('#aprovAtual').val(aprovadores[0].value)
    ...
    $('.aliquota').change(function(){
        const aliquotaValue = parseFloat(this.value);
        if(!isNaN(aliquotaValue)){
            $(this).val(`${aliquotaValue}%`);
        }else{
            $(this).val('');
        }
    })
...
function editParcela(value, index){
    const valueHidden = formataMoedaInt(value)
    $(`#hd_parcelas_valor___${index}`).val(valueHidden)
}
...
//     if ( infoBalance.invoice.length > 0) {
...
function setSelectedZoomItem(selectedItem) {
    ...
    if (value.length > 0) {
        ...
        // //     }
        // } else 
        if (selectedItem.inputName == 'ztxt_dadosnf_natureza') {
            ...
            // Habilita validação da chave de acesso quando natureza for DANFE (começa com 6)
            var codNatureza = selectedItem.CODNATUREZA;
            if(codNatureza.toString().indexOf('6') == 0){
                validaChaveDANFE = true;
            }else{
                validaChaveDANFE = false;
            }
...
function populaDadosAprov(item, beforeSend) {
    try{
        ...
        if (nivel == 'nv1') {
            $('#contadorAprov').val(0)
            if ((item[0]['CHAPASUBST_APROV1'] != 'null' || item[0]['LOGINSUBST_APROV1'] != 'null') && validReplacementDate(item, 1)) {
            ...
            if (error == undefined) {
                if (colleagueId != 'null' && colleagueId != null) {
                    var validaAprovadorDuplicado = Array.from($('[id*=txt_IdAprovador___]')).filter(element => element.value == colleagueId)
                        
                    if(validaAprovadorDuplicado.length == 0){
                        const id = wdkAddChild('tb_IDaprovacoes')
                        $(`#txt_IdAprovador___${id}`).val(colleagueId)
                    }
                } else {
                    if(!beforeSend){
                        FLUIGC.toast({
...
function validReplacementDate(item, indexApprover){
    //Funcao para validar a data de substuição 
    const dates = {
        inicio : new Date(item[0][`TXT_DATADE_SUBST_APROV${indexApprover}`].split('/').reverse().join('-')),
        fim : new Date(item[0][`TXT_DATAATE_SUBST_APROV${indexApprover}`].split('/').reverse().join('-')),
        atual : new Date()
    }

    return dates.atual >= dates.inicio && dates.atual <= dates.fim;
}
...
function limpaHiddensAprov() {
    ...
    $('#aprov3').val('')
    $('#contadorAprov').val('')
    limpa_pai_filho('tb_IDaprovacoes')
    $('#qntAprov').val('')
}
...
function natureza(item, pedido) {
    ...
    function preencher(item, pedido) {
        ...
        var c1 = DatasetFactory.createConstraint('CODDESC', item.CODNATUREZA, item.CODNATUREZA, ConstraintType.MUST)
        var dsNatDespesas = DatasetFactory.getDataset('dsNaturezaDespesa', null, [c1], null)
        var contaDebito = (dsNatDespesas.values[0]['CONTADEBITO'] != 'null') ? dsNatDespesas.values[0]['CONTADEBITO'] : '-------';
        var contaCredito = (dsNatDespesas.values[0]['CONTACREDITO'] != 'null') ? dsNatDespesas.values[0]['CONTACREDITO'] : '-------';
...
function rateio(TipoLancamento, id) {
    var tr = $('#tbparcelas tbody tr')
    if (tr.length > 0) {
        limpa_pai_filho('tbccusto', 'tipolanc')
    }
...
function removedZoomItem(removedItem) {
    ...
    //     validaVl('txt_dadosnf_valorbruto', $('#txt_dadosnf_valorbruto').val(), $('#txt_dadosnf_valorbruto')[0].parentNode)
    //     resetCamposNF()
    //     if ($('#ztxt_solicitacao_fornecedor').val().length == 0 ) {
    //         getFornec()
    //         resetFornec()
    //         window['ztxt_solicitacao_fornecedor'].disable(false)
    //     } } 
        ...
        $('#aprov2').val('')
        $('#aprov3').val('')
        limpa_pai_filho('tb_IDaprovacoes') //limpar tabela de aprovacoes- AJUSTADO
        window['ztxt_ccusto_ccaprov'].clear()
...
function leCSV(evt) {
    ...
    for (var i = 1; i < fileArr.length; i++) {
        ...
        let c1 = DatasetFactory.createConstraint("cod", infoTableCSV.codCCusto, infoTableCSV.codCCusto, ConstraintType.MUST)
        let dts_cc = DatasetFactory.getDataset("dts_CCustoPagNotas_MFX", null, [c1], null).values
        ...
        if(dts_cc.length == 0){
            ...
            setZoomData('txtz_ccusto_cc___' + infoTableCSV.indice, dts_cc[0].FILTRO)
            $("#hdn_codcusto___" + infoTableCSV.indice).val(dts_cc[0].CENTROCUSTO)
        }
...
function recalcular() {
    ...
    if (valortotalalterado > valortotal) {
        ...
        for (var a = 0; a < idsrestantes.length; a++) {
            ...
            editParcela(valorparcela, idsrestantes[a])
        }
    }
}
...
function formatacampo(value, mod, id, obj, divPai, imposto) {
    ...
    if (id == 'txt_dadosnf_valorbruto') {
        ...
        const index = id.split('___')[1];
        editParcela(value, index)
    }
...
function validarChaveAcesso(){
    var retorno = true;
    var chaveAcesso = $("#txt_dadosnf_codAcesso").val();

    if(chaveAcesso.length != 44){
        retorno = false;
    }

    // Extrai os primeiros 43 dígitos da chave de acesso
    var digitos = chaveAcesso.substring(0, 43);
    ...
    // Compara o dígito verificador calculado com o último dígito da chave de acesso
    var ultimoDigito = parseInt(chaveAcesso.charAt(43));
    retorno = (dv === ultimoDigito);
...
var beforeSendValidate = function (numState, nextState) {
    if(numState == 5){
        ...
        if(validaChaveDANFE){
            if( !validarChaveAcesso() ){
                throw 'Chave de acesso inválida, por favor verificar e preencher corretamente.'
            }
        }
    }
    if (numState == 5 || numState == 12 || numState == 20 || (numState == 10 && $('#txta_logError').val() != '')) {
        ...
        $('#tbccusto_index').val('')
        var aprov = $('#ztxt_ccusto_ccaprov').val() //Rateio exceção
        console.log(aprov)
        if (aprov.length == 0) {
            limpa_pai_filho('tb_IDaprovacoes')
            var tr = $('#tbccusto tbody tr')
            ...
            const aprovadores = Array.from($('[id*=txt_IdAprovador___]'));
            var validaAprovadorDuplicado = aprovadores.filter(element => element.value == $('#pacoteiro').val())

            if(validaAprovadorDuplicado.length == 0){
                const id = wdkAddChild('tb_IDaprovacoes')
                $(`#txt_IdAprovador___${id}`).val($('#pacoteiro').val())
            }
        ...
        var aprovAtual = $('[id*=txt_IdAprovador___]')[0].value
            ...
            if ($('#hdn_contabilidade').val() == 'nao') {
                ...
                arrTable.forEach(element => {
                    ...
                    if(cofins != 0 || pis != 0 || csll != 0){
                        contaCredit = '21302001'; //'21402001' Atualizado com leidy 09/04/24
                        imposto = true
                        ...
                        contaCredit = '21302003'; //'21402007'  Atualizado com leidy 09/04/24
                        imposto = true
                        ...
                        contaCredit = '21302002'; //'21402006' Atualizado com leidy 09/04/24
                        imposto = true
                        ...
                        contaCredit = '21402008'; //'21402017' Atualizado com leidy 09/04/24
                        imposto = true
                    ...
                    function validaContaCredit(imposto, dts, contaCredit){
                        if(imposto == true){
                            let validateCredit = dts.filter(element => element.CONTA == contaCredit && element.CLASSIFICACAO == 'C')
...
function getParams() {
    ...
    var valVlInss = $('#' + idVlrInss).val()
    var valAliqInss = $('#' + idAliqInss).val()
    if(valVlInss != '' && valVlInss != null){
        valVlInss = formataMoedaInt(valVlInss);

        if(tipoDoc == 'juridica'){
            prVlrinss = valVlInss;
        }else{
            prVlrinssPesFis = valVlInss;
        }
    }
    if(valAliqInss != '' && valAliqInss != null){
        valAliqInss = parseFloat(valAliqInss.replace('%', ''));

        if(tipoDoc == 'juridica'){
            prAliqinss = valAliqInss;
        }else{
            prAliqinssPesFis = valAliqInss;
        }
    }
    ...
    var valVlIr = $('#' + idVlrIr).val()
    var valAliqIR = $('#' + idAliqIr).val()
    if(valVlIr != '' && valVlIr != null){
        valVlIr = formataMoedaInt(valVlIr);

        if(tipoDoc == 'juridica'){
            prVlrir = valVlIr;
        }else{
            prVlrirPesFis = valVlIr;
        }
    }
    if(valAliqIR != '' && valAliqIR != null){
        valAliqIR = parseFloat(valAliqIR.replace('%', ''));

        if(tipoDoc == 'juridica'){
            prAliqir = valAliqIR;
        }else{
            prAliqirPesFis = valAliqIR;
        }
    }
    ...
    var valBaseInss = $('#' + idBaseInss).val()
    if(valBaseInss != '' && valBaseInss != null){
        valBaseInss = formataMoedaInt(valBaseInss);

        if(tipoDoc == 'juridica'){
            prVlrbaseinss = valBaseInss;
        }else{
            prVlrBaseinssPesFis = valBaseInss;
        }
    }
    ...
    var valBaseIr = $('#' + idBaseIr).val()
    if(valBaseIr != '' && valBaseIr != null){
        valBaseIr = formataMoedaInt(valBaseIr);

        if(tipoDoc == 'juridica'){
            prVlrbaseir = valBaseIr;
        }else{
            prVlrbaseirPesFis = valBaseIr;
        }
    }
    ...
    if (dts[0]['SEQINCONSISTENCIA'] != null) {
        ...
        error = true
        return [dts[0]['SEQINCONSISTENCIA'], dts[0].SEQNOTA]
    ...
    if (error == false) {
        ...
        for (var i = 1; i < tr.length; i++) {
            ...
            var parametrosRateio = prSeqnota + "," + prNroempresa + ",'" + prUsuarioLacto + "'," + pr_valor + "," + prValorPrincipal + "," + prNumeroNF + ",'" + prCodHistorico + "'," + pr_centroCusto + "," + prConta + ",'" + prObservacao + "', ?"
